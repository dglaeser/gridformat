 ...
 private:
    ...
    void _write_field(HighFive::File& file,
        ...
        if (context.is_parallel) {
            ...
            std::vector<std::size_t> offset(layout.dimension(), 0.0);
    ...
    void _visit_field_values(const Field& field, const Visitor& visitor) const {
        field.precision().visit([&] <typename T> (const Precision<T>&) {
            ...
            if (layout.is_scalar()) {
                // TODO: Support scalar? Exception message below says otherwise...
                T data;
    ...
    auto _dump_options() const {
        // TODO: make compression settable (default: off) & don't allow for parallel?
        return H5Easy::DumpOptions(H5Easy::Compression{true});
    ...
    void _dump_dataset(HighFive::File& file,
        ...
        H5Easy::dump(file, groupPrefix + "/" + name, data, _dump_options());
    }
