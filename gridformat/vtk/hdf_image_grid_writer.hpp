...
class VTKHDFImageGridWriter : public GridWriter<Grid> {
    ...
    struct Specs { O origin; S spacing; E extents; };

    template<typename O, typename S, typename E>
    auto specs_from(O&& origin, S&& spacing, E&& extents) const {
        return Specs<std::decay_t<O>, std::decay_t<S>, std::decay_t<E>>{
            .origin = origin,
            .spacing = spacing,
            .extents = extents
        };
 ...
 private:
    ...
    virtual void _write(const std::string& filename_with_ext) const {
        ...
        auto file = open_file(filename_with_ext, _comm, context);

        const auto my_specs = specs_from(origin(this->grid()), spacing(this->grid()), extents(this->grid()));
        ...
        const auto point_slice_base = _make_slice(
            _incremented(overall_specs.extents),
            _incremented(my_specs.extents),
            my_offset
        );

        auto vtk_group = file.createGroup("VTKHDF");
        vtk_group.createAttribute("Version", std::array<std::size_t, 2>{1, 0});
        vtk_group.createAttribute("Origin", _extended(overall_specs.origin));
        vtk_group.createAttribute("Spacing", _extended(overall_specs.spacing));
        ...
        // TODO: There is an issue with reading cell arrays, fixed in https://gitlab.kitware.com/vtk/vtk/-/issues/18860
        //       We should find out which release this is part of, and then write a warning about this in the readme
    ...
    auto _get_image_specs(const Specs<Args...>& specs) const {
        ...
        if constexpr (use_mpi) {
            ...
            return std::make_tuple(
                specs_from(my_whole_origin, specs.spacing, my_whole_extent),
                my_extent_offset
            );
        } else {
    ...
    template<typename R>
    auto _incremented(const R& in) const {
        auto out = in;
        std::ranges::for_each(out, [&] (auto& v) { v += 1; });
        return out;
    }

    // TODO: Put somewhere generic?
    template<std::ranges::range R>
    auto _extended(const R& in) const {
        using T = std::ranges::range_value_t<R>;
        std::array<T, vtk_space_dim> result;
        std::ranges::fill(result, T{0});
    ...
    void _visit_field_values(const Field& field, const Visitor& visitor) const {
        field.precision().visit([&] <typename T> (const Precision<T>&) {
            ...
            const auto grid_extents = extents(this->grid());
            auto size = is_point_field ? _incremented(grid_extents) : grid_extents;
