...
class Appendix {
 ...
 public:
    ...
    void add(DataArray<Args...>&& data_array) {
        _content.emplace_back(_make_ptr(DataArrayImpl{std::move(data_array)}));
    }
 ...
 private:
    ...
    auto _make_ptr(DataArrayImpl<DA>&& impl) const {
        return std::make_unique<DataArrayImpl<DA>>(std::move(impl));
    }
