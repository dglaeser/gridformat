...
def is_branch(name: str) -> bool:
    try:
        subprocess.run(["git", "show-ref", "--verify", f"refs/heads/{name}"], check=True, capture_output=True)
...
clone_repo_to(repo_dir, source_remote=args["source_remote"], target_remote=args["target_remote"])

print("Detecting modified files")
modified_files = get_modified_files_in(
    repo_dir,
    source_tree=f"origin/{args['source']}" if is_branch(args['source']) else args['source'],
    target_tree=f"origin/{args['target']}" if is_branch(args['target']) else args['target']
)

print("Detected the following modified files:/n")
print(" -", "\n - ".join(modified_files))
