...
int main(int argc, char** argv) {
    ...
    GridFormat::PVTRWriter writer{grid, comm};
    GridFormat::PVTRReader reader{comm};
    const auto test_filename = GridFormat::Test::test_reader<2, 2>(
        writer,
        reader,
        "reader_pvtr_test_file_2d_in_2d",
        {},
        (rank == 0)
    );
    ...
    "pvtr_reader_name"_test = [&] () {
        expect(reader.name() == "PVTRReader");
    };

    "parallel_pvtr_read_number_of_pieces"_test = [&] () {
    ...
    if (rank == 0) {
        ...
        "sequential_pvtr_read_field_names"_test = [&] () {
        ...
        "sequential_pvtr_read_point_fields"_test = [&] () {
        ...
        "sequential_pvtr_read_cell_fields"_test = [&] () {
        ...
        const auto seq_filename = vtu_writer.write(
            "reader_pvtr_test_file_2d_in_2d_rewritten_as_vtu_rank_" + std::to_string(rank)
        );
