...
void test(const unsigned int number_of_subdivisions_per_segment) {
    ...
    VTPReader reader;
    auto decorated_reader = ReaderDecorators::PolylinesSubdivider{GridFormat::VTPReader{}};
    reader.open(vtp_filename);
    decorated_reader.open(vtp_filename);

    "number_of_cells_points"_test = [&] () {
        expect(eq(reader.number_of_cells(), grid.cells().size()));
        expect(eq(decorated_reader.number_of_points(), reader.number_of_points()));
        expect(eq(decorated_reader.number_of_cells(), grid.cells().size()*number_of_subdivisions_per_segment));
    };

    "field_names"_test = [&] () {
        expect(std::ranges::equal(cell_field_names(reader), cell_field_names(decorated_reader)));
        expect(std::ranges::equal(point_field_names(reader), point_field_names(decorated_reader)));
        expect(std::ranges::equal(meta_data_field_names(reader), meta_data_field_names(decorated_reader)));
    };
    ...
    "point_field_values"_test = [&] () {
        std::ranges::for_each(point_field_names(reader), [&] (const auto& name) {
            ...
            original_field->precision().visit([&] <typename T> (const Precision<T>&) {
                ...
                const auto decorated = decorated_reader.point_field(name)->serialized();
                expect(std::ranges::equal(original.template as_span_of<T>(), decorated.template as_span_of<T>()));
            });
        });
    };
    ...
    "metadata_field_values"_test = [&] () {
        std::ranges::for_each(meta_data_field_names(reader), [&] (const auto& name) {
            ...
            original_field->precision().visit([&] <typename T> (const Precision<T>&) {
                const auto original = original_field->serialized();
                const auto decorated = decorated_reader.meta_data_field(name)->serialized();
                expect(std::ranges::equal(original.template as_span_of<T>(), decorated.template as_span_of<T>()));
            });
        });
    };
    ...
    "cell_field_values"_test = [&] () {
        std::ranges::for_each(cell_field_names(reader), [&] (const auto& name) {
            ...
            original_field->precision().visit([&] <typename T> (const Precision<T>&) {
                const auto decorated_field = decorated_reader.cell_field(name);
                expect(eq(decorated_field->layout().dimension(), original_field->layout().dimension()));
                expect(eq(decorated_field->layout().extent(0), original_field->layout().extent(0)*number_of_subdivisions_per_segment));
                if (original_field->layout().dimension() > 1)
                    expect(decorated_field->layout().sub_layout(1) == original_field->layout().sub_layout(1));

                const auto original = original_field->serialized();
                const auto decorated = decorated_field->serialized();
                ...
                for (std::size_t orig_cell_idx = 0; orig_cell_idx < original_field->layout().extent(0); ++orig_cell_idx) {
                    const auto decorated_cell_idx = orig_cell_idx*number_of_subdivisions_per_segment;
                    for (std::size_t subdivision = 0; subdivision < number_of_subdivisions_per_segment; ++subdivision) {
                        for (std::size_t i = 0; i < components; ++i)
                            expect(eq(
                                ...
                                decorated.template as_span_of<T>()[(decorated_cell_idx + subdivision)*components + i]
                            )) << " (name = " << name
