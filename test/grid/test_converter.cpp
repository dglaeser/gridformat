...
int main() {
    const GridFormat::Test::StructuredGrid<2> struct_grid{{1.0, 1.0}, {4, 5}};
    const auto struct_data = GridFormat::Test::make_test_data<2>(struct_grid, GridFormat::float64);
    ...
    const auto struct_filename = struct_writer.write("converter_image_vtk_hdf_2d_in_2d_in");
    ...
    GridFormat::convert(reader, "converter_unstructured_vtk_hdf_2d_in_2d_out", [&] (const auto& grid) {
        return GridFormat::VTKHDFUnstructuredGridWriter{grid};
    });

    std::cout << "\"Converting\" image to structured vtk hdf file" << std::endl;
    GridFormat::convert(reader, "converter_image_vtk_hdf_2d_in_2d_out", [&] (const auto& grid) {
        return GridFormat::VTKHDFImageGridWriter{grid};
    });

    std::cout << "\"Converting\" image to vti file" << std::endl;
    GridFormat::convert(reader, "converter_vti_2d_in_2d_out", [&] (const auto& grid) {
        return GridFormat::VTIWriter{grid};
    });

    std::cout << "\"Converting\" image to vtu file" << std::endl;
    GridFormat::convert(reader, "converter_vtu_2d_in_2d_out", [&] (const auto& grid) {
