...
int main(int argc, char** argv) {
    ...
    auto writer = GridFormat::make_writer(grid, MPI_COMM_WORLD);
    ...
    writer.write("generic_parallel_writer_2d_in_2d");

    writer = GridFormat::make_writer(grid, MPI_COMM_WORLD, GridFormat::FileFormat::vtu);
    writer.set_point_field("point_func", [&] (const auto& p) {
        return test_function<double>(GridFormat::coordinates(grid, p));
    });
    writer.set_cell_field("cell_func", [&] (const auto& c) {
    ...
    writer.write("generic_parallel_writer_2d_in_2d_as_vtu");
